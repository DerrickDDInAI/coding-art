...
## toDo:
- to import local modules, we need to add their directory paths sys.path
for now, I use absolute paths but I'd like to use relative paths instead
I tried the following without success: "current_working_directory = Path(__file__).parent.resolve()"
- Same problem to read csv file: use relative path instead of absolute
...
FRAME_START = 0
FRAME_END = 300 # at 30 fps, the number of frames will be 300 + 1
# nb_frames = FRAME_END - FRAME_START
...
blender_utils.set_output_properties(
    filepath="/tmp/",
    file_format="PNG",
    resolution_x=1920, 
    resolution_y=1080, 
